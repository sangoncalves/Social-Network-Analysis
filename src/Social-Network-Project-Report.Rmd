...
Each node in average has under than 50% of connections with each other, so their relationship might not being so strong. Inc a case where we want to analyze the diffusion of a information, we would not have an desirable efficiency due to the lack of connections among the nodes of the network.
...
```
In this scenario we have nodes that do not reciprocate (c,f,h and i) and nodes that own the highest reciprocity (a,d,g). We can understand this network as an unbalanced network in terms of providing advice. Other interpretation is that the nodes *a*,*d* and *g*, are the ones perceived as the ones that holds the best information (advice).
